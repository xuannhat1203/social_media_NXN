...
export default function AdminComments() {
  const getListComment = useSelector((state: any) => state.comment.comment);
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const [render, setRender] = useState(getListComment);
  ...
  const handleSearch = () => {
    if (name.trim() === "") {
      ...
      const find = getListComment.filter((user: any) =>
    ...
    <div className="flex h-screen bg-gradient-to-r from-gray-100 to-gray-300">
      ...
      <main className="flex-1 p-10 overflow-auto">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800">Quản lí comments</h1>
